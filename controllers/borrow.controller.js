const { response } = require("../routes/borrow.route")
...
exports.addBorrowing = async (request, response) => {
    ...
    /** execute for inserting to borrow's table */ borrowModel.create(newData)
        .then(result => {
            ...
            */
            for (let i = 0; i < detailsOfBorrow.length; i++) {
                ...
                //UNTUK NGE LOPP JIKA DATA LEBIH DARI 1
            }

    /** insert all data of detailsOfBorrow */ detailsOfBorrowModel.bulkCreate(detailsOfBorrow)
                .then(result => {
                    //UNTUK MENYIMPAN DATA DETAIL SEBANYAK JUMLAH DATA
    ...
    let data = await borrowModel.findAll(
        {
            include: [
                `member`, `admin`,
                {
                    model: detailsOfBorrowModel, as: `details_of_borrow`, include: ["book"]
                }
            ]
        }
    )
...
return response.json({
    success: true,
    data: data,
    message: "all borrowing have been loaded"
})}